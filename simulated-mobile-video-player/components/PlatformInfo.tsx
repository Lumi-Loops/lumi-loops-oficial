...
const platformDetails: PlatformInfoData = {
    instagram: {
        ...
        renderStyle: 'UI elements and text overlay the video content, with interactive icons on the side and bottom.',
    },
    tiktok: {
        name: 'TikTok Video',
        ...
        renderStyle: 'Heavy UI overlay with creator info, sounds, and interactive icons dominating the screen edges.',
    },
    youtube: {
        name: 'YouTube Shorts',
        ...
        renderStyle: 'Full-screen vertical video with a persistent right-aligned action icon column and bottom user info.',
    },
    snapchat: {
        ...
        renderStyle: 'Ephemeral, full-screen content with a minimalistic and temporary UI overlay for a clean view.',
    },
    twitter: {
        ...
        renderStyle: 'Video is embedded within a tweet card, maintaining the context of the post within a scrollable feed.',
    },
    facebook: {
        name: 'Facebook Video',
        ...
        renderStyle: 'Can be a full-screen Reel or a video embedded in a post card, each with its own interaction patterns.',
    },
    linkedin: {
        name: 'LinkedIn Video',
        ...
        renderStyle: 'Video is a primary element within a professional feed post, often used for presentations or announcements.',
    },
};

const DetailItem: React.FC<{icon: React.ReactNode, title: string, value: string}> = ({ icon, title, value }) => (
...
export const PlatformInfo: React.FC<{ platform: SocialPlatform }> = ({ platform }) => {
    ...
    return (
        <div className="bg-card/40 p-6 rounded-xl">
            ...
            <ul className="space-y-5">
                <DetailItem icon={<RenderIcon />} title="Interface Type" value={details.interfaceType} />
                <DetailItem icon={<ResolutionIcon />} title="Typical Resolution" value={details.resolution} />
                <DetailItem icon={<AspectRatioIcon />} title="Aspect Ratio" value={details.aspectRatio} />
                <DetailItem icon={<FileFormatIcon />} title="Formats" value={details.formats} />
                <DetailItem icon={<FileSizeIcon />} title="Max Size / Length" value={details.maxSize} />
                <DetailItem icon={<RenderIcon />} title="Render Style" value={details.renderStyle} />
            </ul>
        </div>
    );
};